...
<body>
  ...
  <main id="main">
    ...
    <section class="panel data-panel" aria-labelledby="knowledge-recertifications-title">
      <div class="panel-heading"><div><p class="eyebrow">KNOWLEDGE RECERTIFICATION</p><h2 id="knowledge-recertifications-title">治理知识再认证队列</h2></div>
        <label class="compact-filter">状态<select id="knowledge-recertification-status"><option value="">全部</option><option value="awaiting_review">awaiting_review</option><option value="certified">certified</option><option value="rejected">rejected</option><option value="retired">retired</option></select></label>
      </div>
      <div id="knowledge-recertifications" class="alert-list"></div>
      <p id="knowledge-recertifications-empty" class="empty" hidden>当前筛选条件下没有再认证申请。</p>
    </section>
...
function knowledgeRecertificationPath(){const params=new URLSearchParams({limit:'50'});const status=$('knowledge-recertification-status').value;if(status)params.set('status',status);return '/knowledge-recertifications?'+params.toString()}
...
function renderKnowledgeRecertifications(page){const root=$('knowledge-recertifications');clear(root);$('knowledge-recertifications-empty').hidden=page.items.length!==0;page.items.forEach((record)=>{const card=el('article',null,'alert-card '+record.status+(record.decision==='retire'?' critical':''));card.append(el('div',null,'alert-stripe'));const content=document.createElement('div');content.append(el('h3',record.decision+' · '+record.reason));const meta=el('div',null,'alert-meta');meta.append(el('span','状态 '+record.status,'status '+record.status));meta.append(el('span','决策 '+record.decision));meta.append(el('span','复盘 '+record.postmortem_id,'fingerprint'));meta.append(el('span','版本 '+record.postmortem_version));meta.append(el('span','知识 '+record.knowledge_version,'fingerprint'));meta.append(el('span','请求人 '+record.requested_by));meta.append(el('span','请求 '+formatDate(record.requested_at)));meta.append(el('span','质量证据 '+shortHash(record.quality_evidence_fingerprint),'fingerprint'));if(record.reviewed_by)meta.append(el('span','评审人 '+record.reviewed_by));content.append(meta);card.append(content);if(record.status==='awaiting_review'){const actions=el('div',null,'alert-actions');actions.append(button(record.decision==='retire'?'执行退役':'评审',()=>guarded(async()=>{await api('/knowledge-recertifications/'+record.id+'/review',{method:'POST',body:JSON.stringify({expected_version:record.version})});await loadAll()},'再认证评审已完成。'),record.decision==='retire'));card.append(actions)}root.append(card)})}
...
$('knowledge-recertification-status').addEventListener('change',()=>guarded(loadAll));
